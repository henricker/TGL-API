...
test.group('Delete user', (group) => {
  ...
  test('ensure not delete user when not found user with the user_id', async (assert) => {
    const response = await supertest(BASE_URL)
      .delete('/admin/users/delete-user/3')
  ...
  test('ensure not delete user when admin is not authenticated', async (assert) => {
    const response = await supertest(BASE_URL).delete('/admin/users/delete-user/2').expect(401)
  ...
  test('ensure not delete user when user is not admin', async (assert) => {
    const response = await supertest(BASE_URL)
      .delete('/admin/users/delete-user/2')
  ...
  test('ensure delete user when all is fine', async (assert) => {
    const response = await supertest(BASE_URL)
      .delete('/admin/users/delete-user/2')
      .set('Authorization', `Bearer ${jwtAdmin.token}`)

    assert.exists(response.text)
    assert.equal('user deleted', response.text)
  })
})
