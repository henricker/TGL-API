...
test.group('Promote user', (group) => {
  ...
  test('ensure not promote user when not found user with the user_id', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/admin/users/promote-user/3')
  ...
  test('ensure not promote user when admin is not authenticated', async (assert) => {
    const response = await supertest(BASE_URL).patch('/admin/users/promote-user/2').expect(401)
  ...
  test('ensure not promote user when user is not admin', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/admin/users/promote-user/2')
  ...
  test('ensure promote user when all is fine', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/admin/users/promote-user/2')
      .set('Authorization', `Bearer ${jwtAdmin.token}`)

    assert.exists(response.text)
    assert.equal('user promoted', response.text)
  })
})
