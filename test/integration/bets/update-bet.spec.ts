...
test.group('Update bet', (group) => {
  ...
  test('ensure not to update bet when user is not a owner.', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/users/bets/1')
      .set('Authorization', `Bearer ${othetJwt.token}`)
      .send({
        gameId: 1,
        numbers: [2, 3, 7, 5, 10, 6],
      })
      .expect(403)
  ...
  test('ensure not to update bet when user is not authenticated', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/users/bets/1')
      .send({
        gameId: 1,
  ...
  test('ensure not to update bet when bet not exists', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/users/bets/1')
      .send({
        gameId: 7,
    ...
    assert.exists(response.body.errors)
    assert.propertyVal(response.body.errors[0], 'field', 'gameId')
    assert.propertyVal(response.body.errors[0], 'message', 'game not exists')
  })

  test('ensure to update bet when user is authenticated', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/users/bets/1')
      .send({
        gameId: 1,
    ...
    assert.equal('bet updated successfully', response.text)
  })
})
